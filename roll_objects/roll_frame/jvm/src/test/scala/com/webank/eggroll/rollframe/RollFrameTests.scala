...
class RollFrameTests {
  ...
  private val fieldCount = 1000
  private val rowCount = 1000 // total value count = rowCount * fbCount * fieldCount
  // TODO: fbCount means a set of FrameBatch in one partition. fbCount value which larger then one maybe has bugs.
  private val fbCount = 1 // the num of batch
  ...
  def testCreateDataStore(): Unit = {
    ...
    val output1 = clusterManager.getRollFrameStore("c1","test1",StringConstants.FILE)
  ...
  def loadCaches(input: ErStore): ErStore = {
  ...
  def testMapBatch(): Unit = {
    val input = loadCaches(clusterManager.getRollFrameStore("a1", "test1", StringConstants.FILE))
    val output = clusterManager.getRollFrameStore("a1map", "test1", StringConstants.CACHE)
    val rf = new RollFrameClientMode(input)
    val start = System.currentTimeMillis()
    rf.mapBatch({ cb =>
      val schema =
  ...
  def testReduceBatch(): Unit = {
    ...
    val output = clusterManager.getRollFrameStore("a1_reduce", "test1", StringConstants.CACHE)
    val rf = new RollFrameClientMode(input)
    rf.reduce({ (x, y) =>
      try {
  ...
  def testMatMulV1ByMpBatch(): Unit = {
    val input = loadCaches(clusterManager.getRollFrameStore("a1", "test1", StringConstants.FILE))
    val output = clusterManager.getRollFrameStore("a1Matrix", "test1", StringConstants.CACHE)
    val rf = new RollFrameClientMode(input)
    val start = System.currentTimeMillis()
    val matrixCols = 1
    rf.mapBatch({ cb =>
  ...
  def testMatMulV1(): Unit = {
    val input = loadCaches(clusterManager.getRollFrameStore("a1", "test1", StringConstants.FILE))
    val output = clusterManager.getRollFrameStore("a1Matrix1", "test1", StringConstants.CACHE)
    val rf = new RollFrameClientMode(input)
  ...
  @Test
  def testMatMul(): Unit = {
    val input = loadCaches(clusterManager.getRollFrameStore("c1", "test1", StringConstants.FILE))
    val output = clusterManager.getRollFrameStore("c1Matrix1", "test1", StringConstants.CACHE)
    val rf = new RollFrameClientMode(input)
    val matrixRows = fieldCount
    val matrixCols = 1
    val matrix = Array.fill[Double](matrixRows * matrixCols)(1.0)
